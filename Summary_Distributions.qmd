...
subtitle: "List of common distributions and its characteristics"
date: 03-03-2023
date-modified: "Last updated at: `r Sys.Date()`"
freeze: true
format: 
  pdf:
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
	\hline
	\textbf{Distribution}  & \textbf{Parameters} & $f(x)$ & \textbf{Value domain} & \textbf{F(x)} & \textbf{E(X)} & \textbf{Var(X)} & $M_{x}(t) = E(e^{tX})$ & $P_{x}(t)$ \\
	\hline
	...
	\hline
	\makecell{\textbf{Negative Binomial 0} \\ $NB_0(r,p)$} & \makecell{$r = 0, 1, 2, \ldots$  \\  $p \in [0,1]$} & $\binom{x+r-1}{r-1} q^{x} p^r$ & $x = 0, 1,2 ,...$ & $\sum\limits_{i = 1}^{\lfloor k \rfloor} \binom{x+r-1}{r-1} q^{x} p^r$ & $\frac{rq}{p}$ & $\frac{rq}{p^2}$ & $\left(\frac{p}{1-qe^t}\right)^r$ & $\left(\frac{p}{1-qt}\right)^r$ \\
	\hline
	\makecell{\textbf{Negative Binomial 1} \\ $NB_1(r,p)$} & \makecell{$r = 1,2, \ldots$  \\  $p \in [0,1]$} & $\binom{x-1}{r-1} q^{x-r} p^r$ & $x = r, r+1,...$ & \makecell{Complex \\ expression} & $\frac{r}{p}$ & $\frac{rq}{p^2}$ & $\left(\frac{pe^t}{1-qe^t}\right)^r$ & $\left(\frac{pt}{1-qt}\right)^r$  \\
	\hline
	\makecell{\textbf{Hypergeometric} \\ $h(v,a,b)$}& \makecell{$a, b, n$ : pos. integers\\ $n<=a+b$} & $\frac{\binom{a}{x} \binom{b}{n-x}}{\binom{a+b}{n}}$ & x=0,1,2,...n & \makecell{Complex \\ expression} & $\frac{a}{a+b}$ & $\frac{abn(n-1)}{(a+b)^2(a+b-1)}$ & \makecell{Complex \\ expression} & \makecell{Complex \\ expression} \\
	\hline
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
	...
	\hline
	\makecell{\textbf{Uniform} \\ $U(a,b)$} & $-\infty <a<b< +\infty$ & $\Large \frac{1}{b-a}$ & $x \in [a,b]$ & $\begin{cases} 0 , & x<a \\ \frac{x-a}{b-a} , &  a \leq x \leq b \\ 1 , & x>b \end{cases}$ & $\frac{a+b}{2}$ & $\frac{(b-a)^{2}}{12}$ & $\frac{e^{tb} - e^{ta}}{t(b-a)}$ \\
	\hline
	...
	\hline
	\makecell{\textbf{Standard Normal} \\ $N(0,1)$} & \makecell{$\mu = 0$ \\ $\sigma = 1$} & $\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}$ & x = 0,1,... & $\Phi(x)$ & 0 & 1 & $e^{\frac{t^2}{2}}$ \\
	\hline
	...
	\hline
	\makecell{\textbf{Gamma} \\ $Gamma(k,\theta)$} & \makecell{$k > 0$ \\ $\theta$ >0 } & $\frac{x^{k-1}e^{-\frac{x}{\theta}}}{\theta^k \Gamma(k) }$ & $ x \in [0,\infty]$ & $\frac{\gamma\left(k, \frac{x}{\theta}\right)}{\Gamma(k)}$ & $k \theta$ & $k \theta^2$ & $(1-\theta t)^{-k}$ \\
	\hline
	...
	\hline
	\makecell{\textbf{Weibull} \\ $Wei(c, \gamma)$} & \makecell{$c \in (0,+\infty)$ \\ $\gamma \in (0,+\infty)$} & $c\gamma x^{\gamma-1}e^{-cx^{\gamma}}$ & $x \in [0,\infty]$ & $1-exp(-cx^{\gamma})$ & $c$ Γ$(1 + \frac{1}{\gamma})$ & \makecell{Complex \\ expression} & \makecell{Complex \\ expression} \\
	\hline
	...
	\hline
	\makecell{\textbf{Lognormal} \\ $logN(\mu, \sigma^2)$} & \makecell{$\mu \in (-\infty, +\infty)$\\ $\sigma>0$} & $\frac{e^{-\frac{(logx - \mu)^2}{2\sigma^2}}}{x\sigma\sqrt{2\pi}}$ & $x \in (0,+\infty)$ & $\Phi\left(\frac{lnx - \mu}{\sigma}\right)$ & $ e^{\mu + \frac{\sigma^2}{2}} $ & $(e^{\sigma^2}-1)(e^{2\mu+ \sigma^2}) $& --  \\
	\hline

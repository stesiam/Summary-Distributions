...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
	\hline
	\makecell{\textbf{Distribution} \\ \textbf{Συμβολισμός}}  & \textbf{Parameters} & $f(x)$ & \textbf{Value domain} & \textbf{F(x)} & \textbf{E(X)} & \textbf{Var(X)} & $M_{x}(t)$ & $P_{x}(t)$ \\
	\hline
	\makecell{\textbf{Bernoulli} \\ Bernoulli($p$)} & $p \in [0,1]$ & $\begin{cases} q , & \text{εάν k=0} \\ p , & \text{εάν k=1} \end{cases}$ & x = $\{0,1\}$ & $\begin{cases} 0 , & \text{εάν k<0} \\ q , & \text{εάν 0<=k<1} \\ 1 , & \text{εάν k>=1} \end{cases}$ & $p$ & $pq$ & $p(e^{t}+q)$ & $(pt+q)$ \\
	\hline
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
	...
	\hline
	\makecell{\textbf{Stndard Normal} \\ $N(0,1)$} & -- & $\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}$ & x = 0,1,... &  & 0 & 1 &  \\
	\hline
	...
	\hline
	\makecell{\textbf{Erlang} \\ $Erl(k,\lambda)$ \\ εναλλ. $beta = \frac{1}{\lambda}$}  & \makecell{$k \in {1,2,3,...}$  \\ $\lambda \in (0, \infty)$} & $\frac{\lambda^k x^{k-1} e^{-\lambda x}}{(k-1)!}$ & $x \in [0,\infty]$ &  & $\frac{k}{\lambda}$ & $\frac{k}{\lambda^2}$ & \makecell{$(1-\frac{t}{\lambda})^{-k}$ \\ $t<\lambda$} \\
	\hline
